"""
Metrology Vision Pro — Camera Manager Model
Encapsulates IC4 hardware: Grabber, Sink, device enumeration, stream lifecycle.
"""
...
class CameraManager(QObject):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self._sink = ic4.QueueSink(self._listener, [ic4.PixelFormat.BGR8], max_output_buffers=1)
    ...
    def refresh_device_list(self):
        """Re-enumerate physically connected cameras."""
        self._connected_devices = ic4.DeviceEnum.devices()
    ...
    def open_camera(self, serial: str, config_path: str | None = None) -> None:
        ...
        Raises RuntimeError if the camera is not physically connected.
        """
        self.disconnect()
        ...
        device = next((d for d in self._connected_devices if d.serial == serial), None)
        if device is None:
        ...
        try:
            ...
            try:
                ...
                if getattr(e, "code", None) == 34 or "Incomplete" in str(e):
                    print(f"Applied {os.path.basename(path)} with warnings: Some locked properties were ignored.")
                else:
                    raise
