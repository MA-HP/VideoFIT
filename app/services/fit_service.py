    ...
    float delta, float weight, float oob_dist)
{
    ...
    float w = pt_weights[p];
    float v_d = val / delta;
    out_costs[p] = w * weight * delta * delta * (sqrtf(1.0f + v_d * v_d) - 1.0f);
}
    ...
    float delta, float weight, float oob_dist)
{
    ...
    float w = pt_weights[p];
    float v_d = val / delta;
    out_costs[p] = w * (1.0f - expf(-0.5f * v_d * v_d));
}
'''
...
def fit(
    ...
    # -- Step 3: Robust Initial Centroid (Trimmed Median Approach) -----
    if len(edge_points) > 0:
        x_p10, x_p90 = np.percentile(edge_points[:, 0], [10, 90])
        y_p10, y_p90 = np.percentile(edge_points[:, 1], [10, 90])

        valid_x = edge_points[:, 0][(edge_points[:, 0] >= x_p10) & (edge_points[:, 0] <= x_p90)]
        valid_y = edge_points[:, 1][(edge_points[:, 1] >= y_p10) & (edge_points[:, 1] <= y_p90)]

        scene_cx = float(np.median(valid_x)) if len(valid_x) > 0 else W / 2.0
        scene_cy = float(np.median(valid_y)) if len(valid_y) > 0 else H / 2.0
    else:
...
def fit_poc(polylines_all: list[np.ndarray], polylines_rot: list[np.ndarray], polylines_pan: list[np.ndarray],
            edge_points: np.ndarray, silhouette_mask: np.ndarray | None = None,
            distance_field: np.ndarray | None = None,
            objective: str = "Tolerance", max_error_px: float = 1.0) -> FitResult:
    res = fit(polylines_all, edge_points, silhouette_mask, distance_field, objective, max_error_px)
    dist_t_cp = cp.ascontiguousarray(cp.asarray(res.dist_t), dtype=cp.float32)
